...
public class PicCreator {
	...
	public static void generatePicture(ArrayList<Note> notesList)
			throws Exception {
	...
	private static void createTabNumbers(ArrayList<Note> list, int lower,
			int high) {
			...
			for (int i = lower; i < high; i++) {
				if (i == pitch) {
					...
					while (j + 1 < list.size()) {
						...
						if (next.getOnsetInTicks() == note.getOnsetInTicks()
								&& string != 5) {
